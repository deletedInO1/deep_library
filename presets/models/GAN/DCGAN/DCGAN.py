...
class DCGAN(gan.GAN):
    def __init__(self, img_channels=3, z_dim=100, features : GanVar=GanVar(64,64), num_blocks=3, img_size=64):
        ...
        super(DCGAN, self).__init__(Generator(z_dim, img_channels, features.g, num_blocks),
                                    Discriminator(img_channels, features.d, num_blocks), 100, img_channels, img_size)
...
if __name__ == "__main__":
    ...
    z_dim = 100
    dcgan = DCGAN(img_channels=img_channels, z_dim=z_dim, features=GanVar(64, 64), num_blocks=3, img_size=64)
    #print_gen()
    #print_disc()
    test()
